...
export const MobileSearch = forwardRef<MobileSearchHandle, MobileSearchProps>(
	({ isOpen, onClose, categories }, ref) => {
		...
		useEffect(() => {
			...
			// Safari-совместимый автофокус:
			// 1й rAF — браузер рендерит оверлей (clip-path анимация началась)
			// 2й rAF — Safari "видит" элемент как видимый и принимает фокус
			// readOnly-трюк нужен для iOS: без него Safari не поднимает клавиатуру
			// при программном фокусе, если инпут не был readonly→readwrite в том же тике
			...
			// 2. Блокируем вертикальный touch-скролл фона, разрешаем:
			//    - вертикальный скролл результатов (search-panel-scroll)
			//    - горизонтальный скролл фильтров (filters-scroll)
			let touchStartX = 0;
			let touchStartY = 0;
			...
			const preventTouchScroll = (e: TouchEvent) => {
				...
				// Разрешаем скролл внутри результатов поиска
				if (target.closest("[data-slot='search-panel-scroll']")) return;

				// Разрешаем горизонтальный скролл фильтров
				if (target.closest("[data-slot='filters-scroll']")) {
					const touch = e.touches[0];
					if (touch) {
						const dx = Math.abs(touch.clientX - touchStartX);
						const dy = Math.abs(touch.clientY - touchStartY);
						if (dx > dy) return; // горизонтальный свайп — пропускаем
					}
				}
				...
				<div className="shrink-0 flex flex-col w-full backdrop-blur-xl pt-[calc(env(safe-area-inset-top)+0.5rem)] bg-muted-foreground/10 border-b border-foreground/5">
					...
					{state.query.trim().length > 1 && (
						<div
							...
							className="px-1 py-2 w-full overflow-x-auto no-scrollbar animate-in slide-in-from-top-1 duration-150"
						>
							<SearchFilters
